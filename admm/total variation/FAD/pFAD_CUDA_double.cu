...
__device__
double d_NewtonRoot(const double a, const double b, const double c, const double d){
	// initial x = 0.1
	double x = 0.1;
	double x2 = 0.01;
	double x3 = 0.001;
...
__global__
void atomic_reduction_pri(double *d_X, double *d_sol, float *d_rk, float *d_ex, float *d_ez, int imgDim)
{
...
__global__
void atomic_reduction_dual(double *d_sol, double *d_sol_o, double *d_U1, double *d_U2, double *d_U3, float *d_sk, float *d_ed, int imgDim)
{
    int i = threadIdx.x + blockDim.x * blockIdx.x;
    if (i >= imgDim) return;
...
bool stopPri(double *d_X, double *d_sol, int m, int n, double absTol, double relTol, int display)
{
...
__global__
void update_X(double *d_X, double *d_sol, double *d_U, const double rhoinv, const int imgDim, const int p)
{
    int threadId = threadIdx.x + blockDim.x * blockIdx.x;
    if (threadId < imgDim)
...
__global__
void inside_update(double *d_X, unsigned int *d_BlkInd, const int imgHeight, const int imgWidth, const double flam)
{
    int threadId = threadIdx.x + blockDim.x * blockIdx.x;
    int i = threadId % imgHeight;
    int j = threadId / imgHeight;
    ...
    double w0,w1,w2, wt0, wt1, alpha, temp, bb, cc, dd;
    ...
    cc = 2.0*flamp2 * flamp2 * (flamp2 * 12 - wt0 - 3*wt1);
    ...
    d_GGTI[0] = d_GGTI[3] = (alpha + 2.)/temp;
    d_GGTI[1] = d_GGTI[2] = 1./temp;

    /* I - G'inv(GG' + alpha/lamp2*I)G*/
    d_GTGGTI[0] = 1.0 - d_GGTI[0];
    d_GTGGTI[4] = 1.0 - d_GGTI[0] + d_GGTI[1] + d_GGTI[2] - d_GGTI[3];
    d_GTGGTI[8] = 1.0 - d_GGTI[3];
...
__global__
void width_boundary_update(double *d_X, unsigned int *d_BlkInd, const int imgHeight, const int imgWidth, const double flam)
{
    int i = threadIdx.x + blockDim.x * blockIdx.x;
...
__global__
void height_boundary_update(double *d_X, unsigned int *d_BlkInd, const int imgHeight, const int imgWidth, const double flam)
{
...
__global__
void update_sol_u(double *d_X, double *d_sol, double  *d_U1, double *d_U2, double *d_U3, double  *d_Y, int imgDim, double rho)
{
    int i = threadIdx.x + blockDim.x * blockIdx.x;
    if (i >= imgDim) return;
...
int tvl2_iso(double* sol, double* Y, const int imgHeight, const int imgWidth, const double lam, const double rho, const int maxIter, const double absTol, const double relTol, const int display)
{
    ...
    double *d_U1, *d_U2, *d_U3, *d_sol_o, *d_sol;
	double *d_X, *d_Y;
    checkCudaErrors(cudaMalloc((void **) &d_X, 3 * imgDim * sizeof(double)));
    checkCudaErrors(cudaMalloc((void **) &d_sol_o, imgDim * sizeof(double)));
    checkCudaErrors(cudaMalloc((void **) &d_sol, imgDim * sizeof(double)));
    checkCudaErrors(cudaMalloc((void **) &d_Y, imgDim * sizeof(double)));
    checkCudaErrors(cudaMalloc((void **) &d_U1, imgDim * sizeof(double)));
    checkCudaErrors(cudaMalloc((void **) &d_U2, imgDim * sizeof(double)));
    checkCudaErrors(cudaMalloc((void **) &d_U3, imgDim * sizeof(double)));

    checkCudaErrors(cudaMemset((void *) d_U1, 0, imgDim * sizeof(double)));
    checkCudaErrors(cudaMemset((void *) d_U2, 0, imgDim * sizeof(double)));
    checkCudaErrors(cudaMemset((void *) d_U3, 0, imgDim * sizeof(double)));

    checkCudaErrors(cudaMemcpy((void *)d_Y, (void *) Y, imgDim * sizeof(double),cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy((void *)d_sol_o, (void *) Y, imgDim * sizeof(double),cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy((void *)d_sol, (void *) Y, imgDim * sizeof(double),cudaMemcpyHostToDevice));
       ...
       /*determine if the stop conditions are satisfied*/
		...
		if(iter%step_size == 0){
			if(!stopDual(d_sol, d_sol_o, d_U1, d_U2, d_U3, imgHeight, imgWidth, rho,absTol,relTol,display)){
                checkCudaErrors(cudaMemcpy(d_sol_o, d_sol, imgDim * sizeof(double), cudaMemcpyDeviceToDevice));    
				continue;
			}		
    ...
    checkCudaErrors(cudaMemcpy(sol, d_sol_o, imgDim * sizeof(double), cudaMemcpyDeviceToHost));    
