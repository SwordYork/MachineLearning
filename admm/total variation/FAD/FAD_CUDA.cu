 ...
 * no need to do in cuda
 */
float cuda_get_funval(const float *sol, const float *Y, 
        const float lam, const int imgHeight, const int imgWidth)
{
	float obj = 0.;
	float temp = 0.;
	int i, j;
...
/*
 * Newton iteration to solve (31) in paper
 */
__device__
float d_NewtonRoot(const float a, const float b, const float c, const float d)
{
    ...
    // while is inefficient
    // count 15 is best choice according to test
 ...
 * ek = sum(||Xk|| ^ 2)
 * ez = ||Z|| ^ 2
 * because it not the bottleneck, we just do atomicadd 
 */
__global__
void atomic_reduction_pri(float *d_X, float *d_sol, 
        float *d_rk, float *d_ex, float *d_ez, int imgDim)
{
    int i = threadIdx.x + blockDim.x * blockIdx.x;
    if (i >= imgDim) return;

    atomicAdd(d_rk,
            (float) (d_X[i]-d_sol[i])*(d_X[i]-d_sol[i]) + \
            (d_X[imgDim + i]-d_sol[i])*(d_X[imgDim + i]-d_sol[i]) + \
            (d_X[2*imgDim + i] - d_sol[i])*(d_X[2*imgDim + i]-d_sol[i]));

    atomicAdd(d_ex, 
            (float) d_X[i]*d_X[i] + d_X[imgDim + i]*d_X[imgDim + i] + \
            d_X[2*imgDim + i]*d_X[2 * imgDim + i]);

    atomicAdd(d_ez,
            (float) d_sol[i] * d_sol[i]);
}
 ...
 */
__global__
void atomic_reduction_dual(float *d_sol, float *d_sol_o,
        float *d_U1, float *d_U2, float *d_U3, 
        float *d_sk, float *d_ed, int imgDim)
{
    int i = threadIdx.x + blockDim.x * blockIdx.x;
    if (i >= imgDim) return;
    ...
    atomicAdd(d_sk, 
            (float) (d_sol[i]-d_sol_o[i])*(d_sol[i]-d_sol_o[i]));

	atomicAdd(d_ed, 
            (float) d_U1[i]*d_U1[i] + d_U2[i]*d_U2[i] + d_U3[i]*d_U3[i]);
}

/*
 * compute the primal error and residual
 */
bool cuda_stopPri(float *d_X, float *d_sol, 
        int m, int n, float absTol, float relTol, int display)
{
 ...
 */
bool cuda_stopDual(float *d_sol, float *d_sol_o, float *d_U1, float *d_U2, float *d_U3, int m, int n, 
				float rho, float absTol, float relTol, int display)
{
    ...
    // calculate dual error and residual
...
/*
 * only once
 * allocate pixel index in following form:
 * 0 2 1 0
 * 1 0 2 1
 * 2 1 0 2
 * 0 2 1 0 
 ...
 */
__global__
void update_X(float *d_X, float *d_sol, float *d_U, 
        const float rhoinv, const int imgDim, const int p)
{
    int threadId = threadIdx.x + blockDim.x * blockIdx.x;
    if (threadId < imgDim)
...
/*
 * solution to subproblem (15)
 *
 */
__global__
void inside_update(float *d_X, unsigned int *d_BlkInd,
        const int imgHeight, const int imgWidth, const float flam)
{
    int threadId = threadIdx.x + blockDim.x * blockIdx.x;
    int i = threadId % imgHeight;
    int j = threadId / imgHeight;
...
/*
 * according to (32)
 * update X in following form
 */
__global__
void width_boundary_update(float *d_X, unsigned int *d_BlkInd,
        const int imgHeight, const int imgWidth, const float flam)
{
    int i = threadIdx.x + blockDim.x * blockIdx.x;
 ...
 */
__global__
void height_boundary_update(float *d_X, unsigned int *d_BlkInd,
        const int imgHeight, const int imgWidth, const float flam)
{
...
/*
 * according to Algorithm 1 in paper
 */
__global__
void update_sol_u(float *d_X, float *d_sol, 
        float  *d_U1, float *d_U2, float *d_U3, 
        float  *d_Y, int imgDim, float rho)
{
    int i = threadIdx.x + blockDim.x * blockIdx.x;
    if (i >= imgDim) return;
 ...
 */
int cuda_tvl2_iso(float* sol, float* Y, 
        const int imgHeight, const int imgWidth, 
        const float lam, const float rho, 
        const int maxIter, const float absTol, 
        const float relTol, const int display)
{
	const int imgDim = imgHeight * imgWidth;
	const int imgStoreSize= imgHeight * imgWidth * sizeof(float);
    ...
    checkCudaErrors(cudaMalloc((void **) &d_X, 3 * imgStoreSize)); // Z - theta
    checkCudaErrors(cudaMalloc((void **) &d_sol_o, imgStoreSize)); // X prev
    checkCudaErrors(cudaMalloc((void **) &d_sol, imgStoreSize));   // Z
    checkCudaErrors(cudaMalloc((void **) &d_Y, imgStoreSize));     // Y
    checkCudaErrors(cudaMalloc((void **) &d_U1, imgStoreSize));    // theta1
    checkCudaErrors(cudaMalloc((void **) &d_U2, imgStoreSize));    // theta2
    checkCudaErrors(cudaMalloc((void **) &d_U3, imgStoreSize));    // theta3

    // initial theta to 0
    checkCudaErrors(cudaMemset((void *) d_U1, 0, imgStoreSize));
    checkCudaErrors(cudaMemset((void *) d_U2, 0, imgStoreSize));
    checkCudaErrors(cudaMemset((void *) d_U3, 0, imgStoreSize));

    // initial Y, Z, X prev
    checkCudaErrors(cudaMemcpy((void *)d_Y, (void *) Y, imgStoreSize, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy((void *)d_sol_o, (void *) Y, imgStoreSize, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy((void *)d_sol, (void *) Y, imgStoreSize, cudaMemcpyHostToDevice));

    // each pixel index to do admm
    fill_BlkInd<<<blk_grid_size, BLOCK_WIDTH>>>(d_BlkInd, imgHeight, imgDim);

	for(iter = 0; iter<maxIter; iter++)
	{
     
        // update Vk = Z - thetak in paper 
        update_X<<<normal_grid_size, BLOCK_WIDTH>>>(d_X, d_sol, d_U1, rhoinv, imgDim, 0);
        update_X<<<normal_grid_size, BLOCK_WIDTH>>>(d_X, d_sol, d_U2, rhoinv, imgDim, 1);
        update_X<<<normal_grid_size, BLOCK_WIDTH>>>(d_X, d_sol, d_U3, rhoinv, imgDim, 2);

        checkCudaErrors(cudaDeviceSynchronize());

        // solve (15)
        inside_update<<<normal_grid_size, BLOCK_WIDTH>>>(d_X, d_BlkInd, imgHeight, imgWidth, flam); 
        // solve (17)
        ...
        // FAD
        update_sol_u<<<normal_grid_size, BLOCK_WIDTH >>>(d_X, d_sol, d_U1, d_U2, d_U3, d_Y, imgDim, rho);
        checkCudaErrors(cudaDeviceSynchronize());
    ...
    // free memory
    checkCudaErrors(cudaFree(d_BlkInd));
...
Mat cuda_total_variation(Mat image) {
    Mat chans[3];
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        //
        iter[0] = (float)cuda_tvl2_iso(outputImg, inputImg, imgHeight, imgWidth, lam, gamma, maxIter,tol[0],tol[1],display);
        funVal[0] = cuda_get_funval(outputImg,inputImg,lam,imgHeight,imgWidth);
        //
...
int main(int argc, char** argv )
{
    ...
    tv_image = cuda_total_variation(tv_image);
