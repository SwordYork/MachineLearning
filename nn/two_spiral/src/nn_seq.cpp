...
void nn_seq_spiral()
{
    ...
    **************/
    ...
    for(int k = 0; k < max_iter; ++k) {
        ...
        for (int i = 0; i < num_train_data; ++i) {
            ...
            ***************************/
            ...
            for (int j = 0; j < num_hidden_unit; ++j) {
                delta_hidden_output_u[j] = (alpha * delta_hidden_output_u[j] + ingta * delta_output * hidden_y[j] * hidden_y[j]);
                hidden_output_u[j] += delta_hidden_output_u[j];
            }
            ...
            for (int j = 0; j < num_hidden_unit; ++j) {
                ...
                delta_input_hidden_u[0][j] = (alpha * delta_input_hidden_u[0][j] + ingta * delta_hidden[j] * input_y[0] * input_y[0]);
                input_hidden_u[0][j] += delta_input_hidden_u[0][j];

                delta_input_hidden_u[1][j] = (alpha * delta_input_hidden_u[1][j] + ingta * delta_hidden[j] * input_y[1] * input_y[1]);
                input_hidden_u[1][j] += delta_input_hidden_u[1][j];
            }
        }
    ...
    // cout trained data
    std::ofstream outfile("two_spiral_seq_result.txt");
    for (int j = 0; j < num_hidden_unit; ++j) {
